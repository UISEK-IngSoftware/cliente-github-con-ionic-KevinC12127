...
const RepoItem: React.FC<{repo: RepositoryItem; onDelete: (repo: RepositoryItem) => void; onEdit: (repo: RepositoryItem) => void; disabled?: boolean}> = ({ repo, onDelete, onEdit, disabled }) => {
  ...
  return (
    <IonItemSliding>
      ...
      <IonItemOptions side="end">
        ...
        <IonItemOption color="primary" onClick={() => onEdit(repo)} aria-label="Editar" title="Editar" disabled={disabled}>
          <IonIcon icon={pencilOutline} className="repo-action-icon" />
        </IonItemOption>
        {/* Eliminar: solicita confirmación en la página padre (IonAlert) */}
        <IonItemOption color="danger" onClick={() => onDelete(repo)} aria-label="Eliminar" title="Eliminar" disabled={disabled}>
